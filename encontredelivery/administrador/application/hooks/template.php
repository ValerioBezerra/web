...
class Template {
	...
	private function carregarDadosSessao($CI, &$html) {
		$html   = str_replace("{URL_PEDIDO}", site_url('/json/pedido_json/retornar_quantidade_pedido/'.$CI->session->userdata('dlv_id_emp')), $html);
		$html	= str_replace("{DLV_NOME_EMP}", $CI->session->userdata('dlv_nome_emp'), $html);
		$html	= str_replace("{DLV_NOME_USU}", $CI->session->userdata('dlv_nome_usu'), $html);
		
		$html	= str_replace("{DISPLAY_VALLESOFT}", ($CI->session->userdata('dlv_id_emp') == 1)?'':'display: none;', $html);
		$html	= str_replace("{DISPLAY_VALLESOFT_C}", ($CI->session->userdata('dlv_id_emp') == 1)?'display: none;':'', $html);
		...
		$html	= str_replace("{dlv_cadproduto_per}", ($CI->session->userdata('dlv_cadproduto_per') == 1)?'':'display: none;', $html);
		$html	= str_replace("{dlv_altstatusped_per}", (($CI->session->userdata('dlv_altstatusped_per') == 1) || ($CI->session->userdata('dlv_id_emp') == 1))?'':'display: none;', $html);
	}
}